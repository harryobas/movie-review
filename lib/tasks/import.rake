    ...
    CSV.foreach("#{Rails.root}/movies.csv", headers: true, header_converters: converter) do |row|
      ...
      if review && review.size == 1
        ...
        if rev['movie'] == m.movie
          r = Review.new(user: rev['user'], stars: rev['stars'], review: rev['review'])
          m.reviews << r 
        else
          reviews.unshift(rev)
        end
        ...
        if rev1['movie'] == m.movie && rev2['movie'] == m.movie 
          r1 = Review.new(user: rev1['user'], stars: rev1['stars'], review: rev1['review'])
          r2 = Review.new(user: rev2['user'], stars: rev2['stars'], review: rev2['review'])
          m.reviews << r1 
          m.reviews << r2 
        else
          r = Review.new(user: rev1['user'], stars: rev1['stars'], review: rev1['review'])
          m.reviews << r 
